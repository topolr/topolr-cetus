...
(function () {
        ...
        this.load(packetName).done(function () {
            ...
            if (re.length === ths.info.length) {
                ...
                for (var i = 0; i < ths.info.length; i++) {
                    ...
                    if (!packet.requiremapping[d.packet]) {
    ...
    factory.prototype.def = function (obj) {
        ...
        var r = Object.getPrototypeOf(parent), ops = [obj.option], servs = [obj.services || {}];
        ops.push(r.option ? topolr.json.clone(r.option) : {});
        servs.push(r.services ? topolr.json.clone(r.services) : {});
            ...
            for (var i = 0; i < obj.extend.length; i++) {
                ...
                if (q) {
                    ...
                    for (var k = 0; k < m.length; k++) {
                        var opn = m[k];
                        if (opn !== "option" && opn !== "init" && opn !== "services") {
                            parent[opn] = q.prototype[opn];
                        }
                    }
                    ops.push(topolr.json.clone(q.prototype.option));
                    servs.push(topolr.extend({}, q.prototype.services));
                }
            }
        }
        ops.reverse();
        ...
        servs.unshift({});
        servs.unshift(true);
        parent.services = topolr.extend.apply({}, servs);
        var ptm = ["name", "extend", "shortName", "packet", "option", "services"]
        for (var i in obj) {
            if (ptm.indexOf(i) === -1) {
                parent[i] = obj[i];
            }
...
//        console.log(r);
    ...
    template.autocode = function (temp) {
        ...
        for (var i = 0, len = m.length; i < len; i++) {
            ...
            var ct = m[i].code(pt);
            t += ct + "\r\n";
            if (ct.indexOf("__$$node$$" + pt) !== -1 && ct.indexOf("__$$r$$.push(__$$node$$" + pt + ")") === -1) {
                t += "__$$r$$.push(__$$node$$" + pt + ");\r\n";
            }
        }
    ...
    template.prototype._cache = function (data) {
        ...
        if (is.isObject(data) || is.isArray(data)) {
            if (!data["__cacheid__"]) {
                id = util.randomid(8);
                Object.defineProperty(data, "__cacheid__", {
                    enumerable: false,
                    configurable: false,
                    writable: false,
                    value: id
    ...
    template.prototype._macro = function (num, attr) {
        ...
        if (this._macrofn[n.name]) {
            return this._macrofn[n.name].call(this, attr, function (values) {
                if (n.body !== "") {
                    var parameters = [], datas = [], _fps = [], _fdt = [];
                    for (var i in values) {
                        if (ths._parameters.indexOf(i) === -1) {
                            parameters.push(i);
                            datas.push(values[i]);
                        }
                    }
                    _fps = ths._parameters.concat(parameters);
                    var tp = [];
                    for (var i = 0; i < ths._session.length; i++) {
                        if (i > 2) {
                            tp.push(ths._session[i]);
                        }
                    }
                    _fdt = tp.concat(datas);
                    var inner = new template(n.body, ths._macrofn, _fps);
                    inner._caching = ths._caching;
                    return inner.render.apply(inner, _fdt);
                } else {
                    return "";
                }
            }, n.body !== "");
        } else {
            console.error("[topolr] macro can not found [" + n.name + "] template is " + this._source);
            return "";
        }
    };
    ...
    template.prototype.autoDom = function () {
        ...
        return template.exceute.call(this, this._autocodefn, Array.prototype.slice.call(arguments));
    };
    ...
    if (app.option.debug) {
        window["topolr-source"] = source;
        window["topolr-packet"] = packet;
        window["topolr-module"] = module;
        window["topolr-app"] = app;
    }
    var _override = {};
    topolr.overrideModule = function (name, obj) {
        var _module = module.factory.get(name);
        if (_module) {
            for (var i in obj) {
                if (i !== "init" && i !== "option" && i !== "extend") {
                    _module.prototype[i] = obj[i];
                }
            }
        }
    };
